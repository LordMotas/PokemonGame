...
def pbPositionPokemonSprite(sprite,left,top)
  if sprite.bitmap && !sprite.bitmap.disposed?
    sprite.x=left+(128-sprite.src_rect.width)/2
    sprite.y=top+(128-sprite.src_rect.height)/2
  else
    sprite.x=left
    sprite.y=top
  end
...
def pbSpriteSetCenter(sprite,cx,cy)
  ...
  realwidth=sprite.src_rect.width*sprite.zoom_x
