class PokeBattle_Battler
  ...
  attr_accessor :type1
  attr_accessor :type2
  attr_accessor :type3 #pseudo-type for the battler (Forest's Curse, etc)
...
################################################################################
  ...
  @form_change_time=Time.now if !@form_change_time
  
  attr_reader  :form_change_time
  ...
  def hasMegaMessage?(owner)
    if @pokemon
      return (@pokemon.hasMegaMessage?(owner) rescue false)
    end
    return false
  end
  ...
  def getMegaMessage(owner)
    if @pokemon
      return (@pokemon.getMegaMessage(owner) rescue "")
    end
    return ""
  end
  ...
  def weight
    ...
    w*=2 if self.hasWorkingAbility(:HEAVYMETAL) && !@battle.pbCheckOpposingBypassingAbility(self)
    w/=2 if self.hasWorkingAbility(:LIGHTMETAL) && !@battle.pbCheckOpposingBypassingAbility(self)
...
################################################################################
  ...
  def pbInitPermanentEffects
    ...
    @effects[PBEffects::CanBelch]          = false
  end
  ...
  def pbInitEffects(batonpass)
    ...
    @effects[PBEffects::ParentalBondHit]   = -1
    @effects[PBEffects::ItemLost]          = (self.item != 0) ? 1 : 0
    @effects[PBEffects::Electrify]         = false
    @effects[PBEffects::FinalGambit]       = false
    @effects[PBEffects::SpikyShield]       = false
    @effects[PBEffects::Quash]             = false
    @effects[PBEffects::ForceMove]         = false
    @effects[PBEffects::ForceSwitch]       = false
    @effects[PBEffects::LetSwitch]         = false
...
################################################################################
  ...
  def hasMovedSuccessfullyThisRound?
    return false if !@lastRoundMoved
    return @lastRoundMoved==@battle.turncount && @lastMoveUsed != -1
  end
  ...
  def pbSpeed()
    ...
    if self.pbOwnSide.effects[PBEffects::GrassPledge]>0
      speed=(speed/2).floor
    end
    return speed
  end
  
  def hasBypassingAbility(ignorefaint=false) 
     if self.hasWorkingAbility(:MOLDBREAKER,ignorefaint) ||
       self.hasWorkingAbility(:TERAVOLT,ignorefaint) |
       self.hasWorkingAbility(:TURBOBLAZE,ignorefaint)
         return true
      end
    return false 
  end
...
#      PBDebug.log("!!!***Can't faint if already fainted")
    ...
    #Remove Primal Weather
    if @ability==PBAbilities::DELTASTREAM && 
        @battle.weather==PBWeather::DELTASTREAM &&
        !(@battle.pbCheckGlobalAbility(:DELTASTREAM))
      @weather=0
      @battle.pbDisplayBrief("The mysterious air current has dissipated!") if showMessage
    elsif @ability==PBAbilities::PRIMORDIALSEA && 
        @battle.weather==PBWeather::PRIMORDIALSEA &&
        !(@battle.pbCheckGlobalAbility(:PRIMORDIALSEA))
      @weather=0
      @battle.pbDisplayBrief("The heavy rain has lifted!") if showMessage
    elsif @ability==PBAbilities::DESOLATELAND && 
        @battle.weather==PBWeather::DESOLATELAND &&
        !(@battle.pbCheckGlobalAbility(:DESOLATELAND))
      @weather=0
      @battle.pbDisplayBrief("The harsh sunlight faded.") if showMessage
    end
  ...
  def pbAbilitiesOnSwitchIn(onactive)
    ...
    if onactive
      if !(@battle.isPrimordialWeather?)
        ...
        if self.hasWorkingAbility(:SANDSTREAM) && @battle.weather!=PBWeather::SANDSTORM && @battle.weatherduration!=-1
          ...
          if self.hasWorkingItem(:SMOOTHROCK)
            @battle.weatherduration=8
          else
            @battle.weatherduration=5
          end
          ...
          @battle.pbHideEffect(self)
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("A sandstorm kicked up!"))
          else
        ...
        if self.hasWorkingAbility(:DROUGHT) && @battle.weather!=PBWeather::SUNNYDAY && @battle.weatherduration!=-1
          ...
          if self.hasWorkingItem(:HEATROCK)
            @battle.weatherduration=8
          else
            @battle.weatherduration=5
          end
        ...
        if self.hasWorkingAbility(:SNOWWARNING) && @battle.weather!=PBWeather::HAIL && @battle.weatherduration!=-1
          ...
          if self.hasWorkingItem(:ICYROCK)
            @battle.weatherduration=8
          else
            @battle.weatherduration=5
          end
      ...
      if self.hasWorkingAbility(:AIRLOCK)
        ...
        if EFFECTMESSAGES
          ...
          @battle.pbDisplay(_INTL("{1}'s Cloud Nine made the effects of weather disappear.",pbThis))
        end
        PBDebug.log("[#{pbThis}: has Cloud Nine]")
      end
      #Delta Stream
      if self.hasWorkingAbility(:DELTASTREAM) && onactive && @battle.weather!=PBWeather::DELTASTREAM
          @battle.weather=PBWeather::DELTASTREAM
          ...
          PBDebug.log("[#{pbThis}: has Delta Stream]")
      end
      #Primordial Sea
      if self.hasWorkingAbility(:PRIMORDIALSEA) && onactive && @battle.weather!=PBWeather::PRIMORDIALSEA
        @battle.weather=PBWeather::PRIMORDIALSEA
        @battle.weatherduration=-1
        @battle.pbDisplayEffect(self,false)
        @battle.pbCommonAnimation("Rain",nil,nil)
        ...
        if EFFECTMESSAGES
          ...
          @battle.pbDisplay(_INTL("{1}'s Primordial Sea caused a heavy rain to fall.",pbThis))
        end
        PBDebug.log("[#{pbThis}: has Primordial Sea]")
      end
      #Desolate Land
      if self.hasWorkingAbility(:DESOLATELAND) && onactive && @battle.weather!=PBWeather::DESOLATELAND
        @battle.weather=PBWeather::DESOLATELAND
        @battle.weatherduration=-1
        @battle.pbDisplayEffect(self,false)
        @battle.pbCommonAnimation("Sunny",nil,nil)
        ...
        if EFFECTMESSAGES
          ...
          @battle.pbDisplay(_INTL("{1}'s Desolate Land made the sun turn harsh!",pbThis))
        end
        PBDebug.log("[#{pbThis}: has Desolate Land]")
      end
      # Pressure message
      ...
      # Unnerve
      if self.hasWorkingAbility(:UNNERVE)
        @battle.pbDisplayEffect(self)
          ...
          for i in 0...4
            if pbIsOpposing?(i) && !@battle.battlers[i].isFainted?
              choices[choices.length]=i if @battle.battlers[i].ability!=0 &&
                 ...
                 !isConst?(@battle.battlers[i].ability,PBAbilities,:TRACE) 
            end
          end
          if choices.length==0
      ...
      if self.hasWorkingAbility(:DOWNLOAD) && onactive
        ...
        if ospdef>odef
          if !pbTooHigh?(PBStats::ATTACK)
            ...
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1}'s Attack rose!",pbThis))
            else
          ...
          if !pbTooHigh?(PBStats::SPATK)
            ...
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1}'s Special Attack rose!",pbThis))
            else
      ...
      if self.hasWorkingAbility(:FRISK) && @battle.pbOwnedByPlayer?(@index) && onactive
        ...
        frisked=[]
        frisked.push(pbOpposing1) if pbOpposing1.item>0 && !pbOpposing1.isFainted?
        frisked.push(pbOpposing2) if pbOpposing2.item>0 && !pbOpposing2.isFainted?
        if frisked.length>0
          friskedpoke=frisked[0]
          item=friskedpoke.item
          itemname=PBItems.getName(item)
          @battle.pbDisplayEffect(self)
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1} frisked {2} and found its {3}!",pbThis,friskedpoke.pbThis(true),itemname))
          else
            @battle.pbDisplay(_INTL("{1} frisked the foe and found one {2}!",pbThis,itemname))
          end
          if frisked.length>1
          friskedpoke=frisked[1]
          item=friskedpoke.item
          itemname=PBItems.getName(item)
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1} was frisked, revealing its {2}!",friskedpoke.pbThis,itemname))
          else
            @battle.pbDisplay(_INTL("{1} frisked the other foe and found one {2}!",pbThis,itemname))
          end
          end
        end
      end
    ...
    if self.hasWorkingAbility(:FOREWARN) && @battle.pbOwnedByPlayer?(@index) && onactive
      ...
      if moves.length>0
        ...
        @battle.pbDisplayEffect(self)
        if EFFECTMESSAGES
          @battle.pbDisplay(_INTL("It was alerted to {1}'s {2}!",foe.pbThis,movename))
        else
  ...
  def pbEffectsOnDealingDamage(move,user,target,damage)
    ...
    if damage>0 && move.isContactMove?
      if !target.damagestate.substitute
        ...
        if target.hasWorkingAbility(:CUTECHARM) && @battle.pbRandom(10)<3
          if !user.hasWorkingAbility(:OBLIVIOUS) &&
            ...
            @battle.pbDisplayEffect(target)
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1} fell in love!",user.pbThis(true)))
            else
        ...
        if target.hasWorkingAbility(:IRONBARBS,true) && !user.isFainted?
          ...
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1} was hurt!",user.pbThis(true)))
          else
        ...
        if target.hasWorkingAbility(:MUMMY,true) && !user.isFainted?
          if !isConst?(user.ability,PBAbilities,:MULTITYPE) &&
            ...
            @battle.pbDisplayEffect(target)
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1}'s ability became Mummy!",user.pbThis))
        ...
        if target.hasWorkingAbility(:STATIC,true) &&
          ...
          user.pbParalyze(target)
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1} was paralyzed!",user.pbThis(true)))
          else
            @battle.pbDisplay(_INTL("{1}'s {2} paralyzed {3}!  It may be unable to move!",
               target.pbThis,PBAbilities.getName(target.ability),user.pbThis(true)))
          end
        ...
        if target.hasWorkingAbility(:PICKPOCKET)
          if target.item==0 && user.item>0 &&
            ...
            @battle.pbDisplayEffect(user)
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("It stole {1}'s {2}!",
        ...
        if target.hasWorkingAbility(:GOOEY,true) &&
            user.pbCanReduceStatStage?(PBStats::SPEED,false,false,user)
          @battle.pbDisplayEffect(target)
          user.pbReduceStat(PBStats::SPEED,1,true,true,false,user)
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1}'s Speed fell!",user.pbThis))
          else
            @battle.pbDisplay(_INTL("{1}'s {2} lowered {3}'s Speed!",target.pbThis,
            PBAbilities.getName(target.ability),user.pbThis(true)))
          end
          PBDebug.log("[#{target.pbThis}'s Gooey triggered]")
        end
      end
      if user.hasWorkingAbility(:POISONTOUCH,true) &&
        ...
        if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("It poisoned {1}!",target.pbThis(true)))
        else
    ...
    if damage>0
      if !target.damagestate.substitute
          ...
          if user.effects[PBEffects::Disable]<=0 && move.pp>0 && !user.isFainted?
            ...
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1}'s {2} was disabled!",user.pbThis,
                PBMoves.getName(user.effects[PBEffects::DisableMove])))
            else
        ...
        if target.hasWorkingAbility(:JUSTIFIED) &&
          ...
          if target.pbCanIncreaseStatStage?(PBStats::ATTACK)
            ...
            @battle.pbCommonAnimation("StatUp",target,nil)
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1}'s Attack rose!",target.pbThis(true)))
            else
        ...
        # Magician
        if user.hasWorkingAbility(:MAGICIAN) && user.hp>0
          if target.item>0 && user.item==0 &&
             target.effects[PBEffects::Substitute]==0 &&
            ...
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1} stole {2}'s {3}!",user.pbThis,
                target.pbThis(true),PBItems.getName(user.item)))
            else
              @battle.pbDisplay(_INTL("{1} stole {2}'s {3} with Magician!",user.pbThis,
                target.pbThis(true),PBItems.getName(user.item)))
            end
            PBDebug.log("[#{user.pbThis} stole #{PBItems.getName(target.item)}) from #{target.pbThis(true)} with Magician]")  
          end
        end
        if target.hasWorkingAbility(:RATTLED) &&
          ...
          if target.pbCanIncreaseStatStage?(PBStats::SPEED)
            ...
            @battle.pbCommonAnimation("StatUp",target,nil)
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1}'s Speed rose!",target.pbThis))
            else
        ...
        if target.hasWorkingAbility(:WEAKARMOR) && move.pbIsPhysical?(movetype)
          ...
          if target.pbCanReduceStatStage?(PBStats::DEFENSE,false,true)
            ...
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1}'s Defense was lowered!",target.pbThis))
            else
      ...
      if target.hasWorkingItem(:CELLBATTERY) && isConst?(movetype,PBTypes,:ELECTRIC)
        ...
        if target.pbCanIncreaseStatStage?(PBStats::ATTACK)
          target.pbIncreaseStatBasic(PBStats::ATTACK,1)
          @battle.pbCommonAnimation("StatUp",target,nil)
          @battle.pbDisplay(_INTL("The {2} raised {1}'s Attack!",
             target.pbThis,PBItems.getName(target.item)))
          target.pokemon.itemRecycle=target.item
          target.pokemon.itemInitial=0 if target.pokemon.itemInitial==target.item
          target.item=0
      ...
      if target.hasWorkingAbility(:ANGERPOINT)
        ...
        if target.pbCanIncreaseStatStage?(PBStats::ATTACK) &&
          ...
          @battle.pbCommonAnimation("StatUp",target,nil)
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1} maxed its Attack!",target.pbThis))
          else
      ...
      if target.hasWorkingItem(:WEAKNESSPOLICY) && PBTypes.isSuperEffective?(movetype,target.type1,target.type2,target.type3)
        if target.pbCanIncreaseStatStage?(PBStats::ATTACK) || target.pbCanIncreaseStatStage?(PBStats::SPATK)
        @battle.pbDisplay(_INTL("The Weakness Policy raised {1}'s Attack!",target.pbThis(true)))
        @battle.pbDisplay(_INTL("The Weakness Policy raised {1}'s Special Attack!",target.pbThis(true)))
        anim=true
        #if wifi battle
        ...
        target.item=0
          if target.pbCanIncreaseStatStage?(PBStats::ATTACK)
            target.pbIncreaseStat(PBStats::ATTACK,2,false,anim)
            anim=false
            @battle.pbDisplay(_INTL("{1}'s Attack rose sharply!",target.pbThis(true)))
          end
          if target.pbCanIncreaseStatStage?(PBStats::SPATK)
            target.pbIncreaseStat(PBStats::SPATK,2,false,anim)
            @battle.pbDisplay(_INTL("{1}'s Special Attack rose sharply!",target.pbThis(true)))
          end
        @battle.pbSymbiosis(target)
        end
      end
      if target.hasWorkingItem(:LUMINOUSMOSS) && isConst?(movetype,PBTypes,:WATER)
        ...
        if target.pbCanIncreaseStatStage?(PBStats::SPDEF)
          ...
          @battle.pbCommonAnimation("StatUp",target,nil)
          @battle.pbDisplay(_INTL("The {2} raised {1}'s Special Defense!",
             target.pbThis,PBItems.getName(target.item)))
          target.pokemon.itemRecycle=target.item
          target.pokemon.itemInitial=0 if target.pokemon.itemInitial==target.item
          target.item=0
      ...
      if target.hasWorkingItem(:KEEBERRY) && move.pbIsPhysical?(movetype) #physical attack
        if target.pbCanIncreaseStatStage?(PBStats::DEF)
          target.pokemon.itemInitial=0 if target.pokemon.itemInitial==target.item
          target.item=0
          #@battle.pbDisplay(_INTL("{1}'s Kee Berry was activated!",target.pbThis(true)))
          target.pbIncreaseStat(PBStats::DEFENSE,1,false,true)
          @battle.pbDisplay(_INTL("The {2} raised {1}'s Defense!",
             target.pbThis,PBItems.getName(target.item)))
          @battle.pbSymbiosis(target)
        end
      end
      if target.hasWorkingItem(:MARANGABERRY) && move.pbIsSpecial?(movetype) #physical attack
        if target.pbCanIncreaseStatStage?(PBStats::SPDEF)
          target.pokemon.itemInitial=0 if target.pokemon.itemInitial==target.item
          target.item=0
          #@battle.pbDisplay(_INTL("{1}'s Maranga Berry was activated!",target.pbThis(true)))
          target.pbIncreaseStat(PBStats::SPDEF,1,false,true)
          @battle.pbDisplay(_INTL("The {2} raised {1}'s Special Defense!",
             target.pbThis,PBItems.getName(target.item)))
          @battle.pbSymbiosis(target)
        end
      end
      if target.hasWorkingItem(:SNOWBALL) && isConst?(movetype,PBTypes,:ICE)
        PBDebug.log("[#{target.pbThis}'s Snowball triggered]")
        if target.pbCanIncreaseStatStage?(PBStats::ATTACK)
          target.pbIncreaseStatBasic(PBStats::ATTACK,1)
          @battle.pbCommonAnimation("StatUp",target,nil)
          @battle.pbDisplay(_INTL("The {2} raised {1}'s Attack!",
      ...
      if target.hasWorkingItem(:REDCARD) 
        if !target.effects[PBEffects::Ingrain]
          target.pokemon.itemInitial=0 #one time use
          target.item=0
          @battle.pbDisplay(_INTL("{1} held up its Red Card against {2}!",
             target.pbThis,user.pbThis(true)))
          user.effects[PBEffects::ForceSwitch]=true
        end
        @battle.pbSymbiosis(target)
      end
      if target.hasWorkingItem(:EJECTBUTTON)
        target.pokemon.itemInitial=0 #one time use
        target.item=0
        @battle.pbDisplay(_INTL("{1} is switched out with the Eject Button!",
             target.pbThis))
        target.effects[PBEffects::LetSwitch]=true
        @battle.pbSymbiosis(target)
      end
      # Knock Off's effect(?)
    end
...
# see [2024281]&0xF0, [202420C]
      ...
      @battle.pbDisplayEffect(sbattler) if @battle.synchronize[2]!=PBStatuses::FROZEN
      if @battle.synchronize[2]==PBStatuses::POISON &&
         tbattler.pbCanPoisonSynchronize?(sbattler)
        tbattler.pbPoison(sbattler,sbattler.effect[PBEffects::Toxic]>0)
        if sbattler.effectss[PBEffects::Toxic]>0
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1} was badly poisoned!",tbattler.pbThis(true)))
          else
            @battle.pbDisplay(_INTL("{1}'s {2} badly poisoned {3}!",sbattler.pbThis,
               PBAbilities.getName(sbattler.ability),tbattler.pbThis(true)))
          end
        else
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1} was poisoned!",tbattler.pbThis(true)))
          else
        ...
        if EFFECTMESSAGES
          @battle.pbDisplay(_INTL("{1} was burned!",tbattler.pbThis(true)))
        else
        ...
        if EFFECTMESSAGES
          @battle.pbDisplay(_INTL("{1} was paralyzed!",tbattler.pbThis(true)))
        else
          @battle.pbDisplay(_INTL("{1}'s {2} paralyzed {3}!  It may be unable to move!",
...
################################################################################
  ...
  def pbBerryCureCheck(hpcure=false)
    ...
    if self.hasWorkingItem(:MENTALHERB) &&
      ...
      @battle.pbDisplay(_INTL("{1}'s move is no longer disabled!",pbThis)) if @effects[PBEffects::Disable]>0
    ...
    if berryconsumed && !self.isFainted?
      @effects[PBEffects::CanBelch] = true
    end
    #Cheek Pouch restores 1/3 health if berry was consumed.
    if berryconsumed && self.hasWorkingAbility(:CHEEKPOUCH) && !self.isFainted?
      @battle.pbDisplayEffect(self)
      self.pbRecoverHP((self.totalhp/3).floor,true)
      if EFFECTMESSAGES
        @battle.pbDisplay(_INTL("{1}'s HP was restored.",pbThis)) 
  ...
  def pbChangeTarget(thismove,userandtarget,targets)
    ...
    if targets.length==1 && isConst?(thismove.type,PBTypes,:ELECTRIC) && 
      ...
      for i in priority # use Pokémon earliest in priority
        ...
        if i.hasWorkingAbility(:LIGHTNINGROD) && !user.hasBypassingAbility
    ...
    if targets.length==1 && isConst?(thismove.type,PBTypes,:WATER) && 
      ...
      for i in priority # use Pokémon earliest in priority
        next if !pbIsOpposing?(i.index)
        if i.hasWorkingAbility(:STORMDRAIN) && !user.hasBypassingAbility
    ...
    if changeeffect==1
      ...
      PBDebug.log("[#{target.pbThis}'s Lightning Rod drew the move in]")
...
################################################################################
  ...
  def pbSuccessCheck(thismove,user,target,accuracy=true)
    ...
    if target.effects[PBEffects::KingsShield] && thismove.canProtectAgainst? && 
      !target.effects[PBEffects::ProtectNegation] && thismove.isContactMove?
      @battle.pbDisplay(_INTL("{1} protected itself!",target.pbThis))
        if pbCanReduceStatStage?(PBStats::ATTACK,true,false,target)
          if user.pbTooLow?(PBStats::ATTACK)
            @battle.pbDisplay(_INTL("{1}'s Attack won't go lower!",user.pbThis))
          else
            pbReduceStat(PBStats::ATTACK,2,false)
            @battle.pbDisplay(_INTL("{1}'s Attack harshly fell!",user.pbThis))
            #Insert StatDown Animation
          end
        end
      user.pbCancelMoves
      @battle.successStates[user.index].protected=true
      return false
      PBDebug.log("[#{user.pbThis}: King's Shield stopped the attack]")
    end
    if target.effects[PBEffects::SpikyShield] && thismove.canProtectAgainst? && 
      ...
      if !target.isFainted?
        ...
        @battle.pbDisplay(_INTL("{1} was hurt!",target.pbThis))
        user.pbCancelMoves
        @battle.successStates[user.index].protected=true
        return false
        PBDebug.log("[#{user.pbThis}: Spiky Shield stopped the attack]")
      end      
    end
    #Crafty Shield doesn't block damaging moves
    if target.pbOwnSide.effects[PBEffects::CraftyShield] && !target.effects[PBEffects::ProtectNegation] &&
        thismove.basedamage==0 && thismove.canProtectAgainst?
      @battle.pbDisplay(_INTL("Crafty Shield protected {1}!",target.pbThis(true)))
      PBDebug.log("[#{user.pbThis}: Crafty Shield stopped the attack]")
      return false
    end
    # Quick Guard blocks priority moves
    if target.pbOwnSide.effects[PBEffects::QuickGuard] && thismove.priority>0 &&
        !target.effects[PBEffects::ProtectNegation]
      @battle.pbDisplay(_INTL("Quick Guard protected {1}!",target.pbThis(true)))
      PBDebug.log("[#{user.pbThis}: Quick Guard stopped the attack]")
      return false
    end
    # Wide Guard blocks multi-targeting moves
    if target.pbOwnSide.effects[PBEffects::WideGuard] && [PBTargets::AllNonUsers,PBTargets::AllOpposing].include?(thismove.target) &&
        !target.effects[PBEffects::ProtectNegation] && thismove.basedamage>0
      @battle.pbDisplay(_INTL("Wide Guard protected {1}!",target.pbThis(true)))
      PBDebug.log("[#{user.pbThis}: Wide Guard stopped the attack]")
      return false
    end
    #Mat Block doesn't block status moves
    if target.effects[PBEffects::MatBlock] && !target.effects[PBEffects::ProtectNegation] &&
        thismove.basedamage>0
      @battle.pbDisplay(_INTL("{1} was blocked by the kicked-up mat!",thismove.name))
      PBDebug.log("[#{user.pbThis}: Mat Block stopped the attack]")
      return false
    end
    ...
    if thismove.basedamage>0 && thismove.function!=0x02 && # Struggle
      ...
      if isConst?(type,PBTypes,:GROUND) && target.isAirborne? &&
        ...
        if target.hasWorkingAbility(:LEVITATE) && !user.hasBypassingAbility #Moldbreaker
          @battle.pbDisplayEffect(target)
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("It doesn't affect {1}...",target.pbThis))
          else
  ...
  def pbTryUseMove(choice,thismove,turneffects)
    ...
    if @effects[PBEffects::SkyDrop]
      @battle.pbDisplay(_INTL("{1} can't move!",pbThis))
      PBDebug.log("[#{pbThis} can't move due to being Sky Dropped]")
      return false
    end
    ...
    if @effects[PBEffects::HealBlock]>0 && thismove.isHealingMove?
      @battle.pbDisplay(_INTL("{1} can't use {2} because of Heal Block!",
         pbThis,thismove.name))
    ...
    if @effects[PBEffects::Torment] && thismove.id==@lastMoveUsed &&
      ...
      pbDisplayPaused(_INTL("{1} can't use the same move twice in a row due to the torment!",
         pbThis))
    ...
    if @effects[PBEffects::Flinch]
      @effects[PBEffects::Flinch]=false
      if self.hasWorkingAbility(:INNERFOCUS) && !@battle.pbCheckOpposingBypassingAbility(self) #Moldbreaker
        if !EFFECTMESSAGES
        ...
        if self.hasWorkingAbility(:STEADFAST)
          ...
          if pbCanIncreaseStatStage?(PBStats::SPEED)
            ...
            if EFFECTMESSAGES
              @battle.pbDisplay(_INTL("{1}'s Speed rose!",pbThis))
            else
                @battle.pbDisplay(_INTL("{1}'s {2} raised its speed!",
                   self.pbThis,PBAbilities.getName(self.ability)))
            end
    ...
    # Powder
    if @effects[PBEffects::Powder]
      @effects[PBEffects::Powder]=false
      if thismove.type==getConst(PBTypes,:FIRE)
        @battle.pbDisplay(_INTL("{1} used {2}!",self.pbThis,thismove.name)) #not sure if there's a better method for displaying the "X used Y!" message, but this works
        @battle.pbDisplay(_INTL("When the flame touched the powder on the Pokémon, it exploded!"))
        if self.status==PBStatuses::FROZEN
          self.status=0
          @battle.pbDisplay(_INTL("{1} thawed out!",pbThis))
        end #not actually sure if defrosting happens before or after the messages, but after makes sense for now
        PBDebug.log("[#Powder exploded on {pbThis} and it couldn't move]")
        pbReducePP(thismove)
        pbReduceHP(@totalhp/4,true) if !hasWorkingAbility(:MAGICGUARD)   
        return false
      end
    end
  ...
  def pbProcessMoveAgainstTarget(thismove,user,target,numhits,turneffects,nocheck=false,alltargets=nil,showanimation=true)
    ...
    destinybond=false
    for i in 0...numhits
      next if user.status==PBStatuses::SLEEP || user.status==PBStatuses::FROZEN  #cancel if they fell asleep or were frozen
      ...
      realnumhits+=1
      if user.effects[PBEffects::ParentalBondHit]>-1
        user.effects[PBEffects::ParentalBondHit]+=1
      end
      ...
      if target.damagestate.calcdamage>0 &&
        ...
        addleffect*=2 if user.hasWorkingAbility(:SERENEGRACE) || user.pbOwnSide.effects[PBEffects::WaterPledge]>0
      ...
      if target.damagestate.calcdamage>0 && !target.damagestate.substitute
        if !target.hasWorkingAbility(:SHIELDDUST) || user.hasBypassingAbility
    ...
    user.effects[PBEffects::ParentalBondHit]=-1 #Reset Parental Bond
    user.effects[PBEffects::MeFirst]=false #Reset Me First
    ...
    # Final Gambit
    if !user.isFainted? && user.effects[PBEffects::FinalGambit] 
      user.effects[PBEffects::FinalGambit] =false
    ...
    # Moxie
    if user.hasWorkingAbility(:MOXIE) && !user.isFainted? && target.isFainted?
        if !user.pbTooHigh?(PBStats::ATTACK)
          @battle.pbDisplayEffect(user)
          user.pbIncreaseStat(PBStats::ATTACK,1,false,true,target)
          if EFFECTMESSAGES
            @battle.pbDisplay(_INTL("{1}'s Attack rose!",user.pbThis(true)))
          else
    ...
    if target.hasWorkingAbility(:COLORCHANGE) && totaldamage>0 &&
      ...
      if EFFECTMESSAGES
        @battle.pbDisplay(_INTL("{1} transformed into the {2} type!",target.pbThis,PBTypes.getName(movetype)))
      else
  ...
  def pbUseMove(choice,specialusage=false)
    ...
    for poke in @battle.battlers
      if poke.effects[PBEffects::ForceSwitch]
        poke.effects[PBEffects::ForceSwitch]=false
        poke.effects[PBEffects::LetSwitch]=false # Should already be the case
        if !poke.isFainted?
          choices=[]
          party=@battle.pbParty(poke.index)
          for i in 0..party.length-1
            choices[choices.length]=i if @battle.pbCanSwitchLax?(poke.index,i,false)
          end
          if choices.length>0
            newpoke=choices[@battle.pbRandom(choices.length)]
            poke.pbResetForm
            @battle.pbReplace(poke.index,newpoke,false)
            @battle.pbDisplay(_INTL("{1} was dragged out!",poke.pbThis))
            @battle.pbOnActiveOne(poke)
            poke.pbAbilitiesOnSwitchIn(true)
          end
        end
      end
    end
    #Volt Switch, etc
    priority=(!(@battle.currentpriority.nil?))? @battle.currentpriority : @battle.pbPriority
    for i in priority
      if i.effects[PBEffects::LetSwitch] 
        if !i.isFainted? && @battle.pbCanChooseNonActive?(i.index) &&
           !@battle.pbAllFainted?(@battle.pbParty(i.index))
          # TODO: Pursuit should go here, and negate this effect if it KO's attacker
          for j in priority
            next if !i.pbIsOpposing?(j.index)
            next if j.hasMovedThisRound?
            # if Pursuit and this target ("i") was chosen
            if @battle.pbChoseMoveFunctionCode?(j.index,0x88) &&
              ...
              if j.status!=PBStatuses::SLEEP &&
                ...
                j.lastRoundMoved=@battle.turncount
          ...
          if !i.isFainted?
            ...
            i.pbResetForm
            @battle.pbReplace(i.index,newpoke,true)
