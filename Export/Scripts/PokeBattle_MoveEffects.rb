...
class PokeBattle_Move_003 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.pbCanSleep?(true,false,false,self,attacker)
  ...
  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanSleep?(false,false,false,self,attacker)
      opponent.pbSleep
      @battle.pbDisplay(_INTL("{1} went to sleep!",opponent.pbThis))
      return true
    end
...
class PokeBattle_Move_004 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return -1 if !opponent.pbCanSleep?(true,false,false,self,attacker)
...
class PokeBattle_Move_005 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanPoison?(true,self,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_007 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanParalyze?(true,self,attacker)
  ...
  def pbAdditionalEffect(attacker,opponent)
    return false if !opponent.pbCanParalyze?(false,self,attacker)
    opponent.pbParalyze(attacker)
...
class PokeBattle_Move_009 < PokeBattle_Move
  def pbAdditionalEffect(attacker,opponent)
    ...
    if @battle.pbRandom(10)==0
      break if !opponent.pbCanParalyze?(false,self,attacker)
      opponent.pbParalyze(attacker)
      @battle.pbDisplay(_INTL("{1} was paralyzed!  It may be unable to move!",opponent.pbThis))
      hadeffect=true
    end
...
class PokeBattle_Move_00B < PokeBattle_Move
  def pbAdditionalEffect(attacker,opponent)
    hadeffect=false
    if @battle.pbRandom(10)==0
      break if !opponent.pbCanBurn?(false,self,attacker)
      opponent.pbBurn(attacker)
      @battle.pbDisplay(_INTL("{1} was burned!",opponent.pbThis))
      hadeffect=true
    end
...
class PokeBattle_Move_00C < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanFreeze?(true,self,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    opponent.pbFreeze
  ...
  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanFreeze?(false,self,attacker)
      opponent.pbFreeze
      @battle.pbDisplay(_INTL("{1} was frozen solid!",opponent.pbThis))
      return true
    end
...
class PokeBattle_Move_00E < PokeBattle_Move
  def pbAdditionalEffect(attacker,opponent)
    hadeffect=false
    if @battle.pbRandom(10)==0
      break if !opponent.pbCanFreeze?(false,self,attacker)
      opponent.pbFreeze
      @battle.pbDisplay(_INTL("{1} was frozen solid!",opponent.pbThis))
      hadeffect=true
    end
...
class PokeBattle_Move_013 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    if opponent.pbCanConfuse?(true,attacker)
      pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
  ...
  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanConfuse?(false,attacker)
      if isConst?(attacker.species,PBSpecies,:CHATOT) &&
         ...
         (!opponent.hasWorkingAbility(:SHIELDDUST) || attacker.hasBypassingAbility)
        chance=0
...
class PokeBattle_Move_016 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if @battle.pbCheckSideAbility(:AROMAVEIL,opponent)!=nil
  ...
  def pbAdditionalEffect(attacker,opponent)
    ...
    case rnd
      when 0
        ...
        @battle.pbDisplay(_INTL("{1} was burned!",opponent.pbThis))
      when 1
        return false if !opponent.pbCanFreeze?(false,self,attacker)
        opponent.pbFreeze
        @battle.pbDisplay(_INTL("{1} was frozen solid!",opponent.pbThis))
      when 2
        return false if !opponent.pbCanParalyze?(false,self,attacker)
...
class PokeBattle_Move_01B < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.status==0 ||
      (attacker.status==PBStatuses::PARALYSIS && !opponent.pbCanParalyze?(false,self,attacker)) ||
      (attacker.status==PBStatuses::SLEEP && !opponent.pbCanSleep?(false,false,false,self,attacker)) ||
      (attacker.status==PBStatuses::POISON && !opponent.pbCanPoison?(false,self,attacker)) ||
      (attacker.status==PBStatuses::BURN && !opponent.pbCanBurn?(false,self,attacker)) ||
      (attacker.status==PBStatuses::FROZEN && !opponent.pbCanFreeze?(false,self,attacker))
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    case attacker.status
      ...
      when PBStatuses::FROZEN
        ...
        attacker.status=0
        @battle.pbDisplay(_INTL("{1} thawed out!",attacker.pbThis))
    end
    return 0
  end
end
...
class PokeBattle_Move_01C < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::ATTACK,1,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false,attacker)
      attacker.pbIncreaseStat(PBStats::ATTACK,1,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_01D < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::DEFENSE,1,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false,attacker)
      attacker.pbIncreaseStat(PBStats::DEFENSE,1,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_01F < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::SPEED,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::SPEED,1,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPEED,1,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_020 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::SPATK,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::SPATK,1,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::SPATK,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPATK,1,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_021 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbCanIncreaseStatStage?(PBStats::SPDEF,true,attacker)
      pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
      attacker.pbIncreaseStat(PBStats::SPDEF,1,true,true,attacker)
    end
    return 0
  end
end
...
class PokeBattle_Move_022 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::EVASION,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::EVASION,1,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::EVASION,false,attacker)
      attacker.pbIncreaseStat(PBStats::EVASION,1,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_024 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    showanim=true
    if attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false,attacker)
      attacker.pbIncreaseStat(PBStats::ATTACK,1,true,showanim,attacker)
      showanim=false
    end
    if attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false,attacker)
      attacker.pbIncreaseStat(PBStats::DEFENSE,1,true,showanim,attacker)
      showanim=false
    end
    return 0
  end
...
class PokeBattle_Move_025 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false,attacker) &&
       !attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false,attacker) &&
       !attacker.pbCanIncreaseStatStage?(PBStats::ACCURACY,false,attacker)
      @battle.pbDisplay(_INTL("{1}'s stats won't go any higher!",attacker.pbThis))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ...
    if attacker.pbCanIncreaseStatStage?(PBStats::ACCURACY,false,attacker)
      attacker.pbIncreaseStat(PBStats::ACCURACY,1,true,showanim,attacker)
      showanim=false
    end
    return 0
  end
...
class PokeBattle_Move_026 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPEED,1,true,showanim,attacker)
      showanim=false
    end
    return 0
  end
...
class PokeBattle_Move_027 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if !attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false,attacker) &&
       !attacker.pbCanIncreaseStatStage?(PBStats::SPATK,false,attacker)
      @battle.pbDisplay(_INTL("{1}'s stats won't go any higher!",attacker.pbThis))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ...
    if attacker.pbCanIncreaseStatStage?(PBStats::SPATK,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPATK,1,true,showanim,attacker)
      showanim=false
    end
    return 0
  end
...
class PokeBattle_Move_028 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false,attacker)
      attacker.pbIncreaseStat(PBStats::ATTACK,increment,true,showanim,attacker)
      showanim=false
    end
    if attacker.pbCanIncreaseStatStage?(PBStats::SPATK,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPATK,increment,true,showanim,attacker)
      showanim=false
    end
    return 0
  end
...
class PokeBattle_Move_02A < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbCanIncreaseStatStage?(PBStats::SPDEF,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPDEF,1,true,showanim,attacker)
      showanim=false
    end
    return 0
  end
...
class PokeBattle_Move_02B < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false)
      attacker.pbIncreaseStat(PBStats::SPEED,1,true,showanim)
      showanim=false
    end
    return 0
  end
...
class PokeBattle_Move_02E < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::ATTACK,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false,attacker)
      attacker.pbIncreaseStat(PBStats::ATTACK,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_02F < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::DEFENSE,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false,attacker)
      attacker.pbIncreaseStat(PBStats::DEFENSE,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_030 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::SPEED,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::SPEED,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPEED,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_032 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::SPATK,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::SPATK,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::SPATK,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPATK,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_033 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::SPDEF,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::SPDEF,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::SPDEF,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPDEF,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_034 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::EVASION,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::EVASION,2,true,true,attacker)
  ...
  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::EVASION,false,attacker)
      attacker.pbIncreaseStat(PBStats::EVASION,2,true,true,attacker)
...
class PokeBattle_Move_035 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbCanIncreaseStatStage?(PBStats::SPATK,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPATK,2,true,showanim,attacker)
      showanim=false
    end
    if attacker.pbCanIncreaseStatStage?(PBStats::SPEED,false,attacker)
...
class PokeBattle_Move_037 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbIncreaseStat(stat,2,true,true,attacker)
    return 0
  end
end
...
class PokeBattle_Move_038 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::DEFENSE,3,false,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false,attacker)
      attacker.pbIncreaseStat(PBStats::DEFENSE,3,false,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_039 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    return -1 if !attacker.pbCanIncreaseStatStage?(PBStats::SPATK,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=attacker.pbIncreaseStat(PBStats::SPATK,3,false,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false,attacker)
      attacker.pbIncreaseStat(PBStats::SPATK,3,false,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_03B < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.damagestate.calcdamage>0
      showanim=true
      if attacker.pbCanReduceStatStage?(PBStats::ATTACK,false,true,attacker)
        attacker.pbReduceStat(PBStats::ATTACK,1,true,showanim,true,attacker)
        showanim=false
      end
      if attacker.pbCanReduceStatStage?(PBStats::DEFENSE,false,true,attacker)
        attacker.pbReduceStat(PBStats::DEFENSE,1,true,showanim,true,attacker)
...
class PokeBattle_Move_03D < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if attacker.pbPartner && !attacker.pbPartner.isFainted?
      attacker.pbPartner.pbReduceHP(attacker.pbPartner.totalhp/16,true)
    end
    return ret
  end
end
...
class PokeBattle_Move_03E < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.damagestate.calcdamage>0
      if attacker.pbCanReduceStatStage?(PBStats::SPEED,false,true,attacker)
        attacker.pbReduceStat(PBStats::SPEED,1,true,true,true,attacker)
      end
    end
    return ret
  end
...
class PokeBattle_Move_03F < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.damagestate.calcdamage>0
      if attacker.pbCanReduceStatStage?(PBStats::SPATK,false,true,attacker)
        attacker.pbReduceStat(PBStats::SPATK,2,true,true,true,attacker)
      end
    end
    return ret
  end
...
class PokeBattle_Move_040 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    ret=-1
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.pbCanIncreaseStatStage?(PBStats::SPATK,false,attacker)
      opponent.pbIncreaseStat(PBStats::SPATK,1,true,true,attacker)
      ret=0
    end
    if opponent.pbCanConfuse?(true,attacker)
      opponent.effects[PBEffects::Confusion]=2+@battle.pbRandom(4)
...
class PokeBattle_Move_041 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.pbCanIncreaseStatStage?(PBStats::ATTACK,false,attacker)
      opponent.pbIncreaseStat(PBStats::ATTACK,2,true,false,attacker)
      ret=0
    end
    if opponent.pbCanConfuse?(true,attacker)
      opponent.effects[PBEffects::Confusion]=2+@battle.pbRandom(4)
...
class PokeBattle_Move_042 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::ATTACK,1,true,true,false,attacker)
    return ret ? 0 : -1
  end
...
class PokeBattle_Move_043 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::DEFENSE,true,false,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::DEFENSE,1,true,true,false,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::DEFENSE,false,false,attacker)
      opponent.pbReduceStat(PBStats::DEFENSE,1,true,true,false,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_044 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::SPEED,true,false,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::SPEED,1,true,true,false,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::SPEED,false,false,attacker)
      opponent.pbReduceStat(PBStats::SPEED,1,true,true,false,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_045 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::SPATK,1,true,true,false,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::SPATK,false,false,attacker)
      opponent.pbReduceStat(PBStats::SPATK,1,true,true,false,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_046 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::SPDEF,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::SPDEF,1,true,true,false,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::SPDEF,false,attacker)
      opponent.pbReduceStat(PBStats::SPDEF,1,true,true,false,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_047 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::ACCURACY,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::ACCURACY,1,true,true,false,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::ACCURACY,false,attacker)
      opponent.pbReduceStat(PBStats::ACCURACY,1,true,true,false,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_048 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::EVASION,1,true,true,false,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::EVASION,false,attacker)
      opponent.pbReduceStat(PBStats::EVASION,1,true,true,false,attacker)
    end
    return true
  end
end
...
# hazards for the target's side and all entry hazards on the user's side.
################################################################################
class PokeBattle_Move_049 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    #clear target's entry hazards
    if opponent.pbOwnSide.effects[PBEffects::Spikes] > 0
      opponent.pbOwnSide.effects[PBEffects::Spikes] = 0
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("The spikes disappeared from around the opposing team's feet!"))
      else
    ...
    if opponent.pbOwnSide.effects[PBEffects::ToxicSpikes] > 0
      ...
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("The poisoned spikes disappeared from around the opposing team's feet!"))
      else
        @battle.pbDisplay(_INTL("The poisoned spikes disappeared from around the your team's feet!"))
      end     
    end
    if opponent.pbOwnSide.effects[PBEffects::StickyWeb]
      opponent.pbOwnSide.effects[PBEffects::StickyWeb] = false
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("The sticky web has disappeared from beneath the opposing team's feet!"))
      else
        @battle.pbDisplay(_INTL("The sticky web has disappeared from beneath your team's feet!"))
      end    
    end
    #clear user's entry hazards
    #testing in Gen VI seems to show that it only removes entry hazards
    #and leaves the user's own screens intact
    ...
    if attacker.pbOwnSide.effects[PBEffects::ToxicSpikes] > 0
      ...
      if !@battle.pbIsOpposing?(attacker.index)
        @battle.pbDisplay(_INTL("The poisoned spikes disappeared from around your team's feet!"))
      else
        @battle.pbDisplay(_INTL("The poisoned spikes disappeared from around the opposing team's feet!"))
      end     
    end
...
class PokeBattle_Move_04A < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if opponent.pbReduceStat(PBStats::DEFENSE,1,true,showanim,false,attacker)
      ret=0; showanim=false
    end
    return ret
  end
...
class PokeBattle_Move_04B < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::ATTACK,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::ATTACK,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::ATTACK,false,attacker)
      opponent.pbReduceStat(PBStats::ATTACK,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_04C < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::DEFENSE,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::DEFENSE,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::DEFENSE,false,attacker)
      opponent.pbReduceStat(PBStats::DEFENSE,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_04D < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::SPEED,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::SPEED,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::SPEED,false,attacker)
      opponent.pbReduceStat(PBStats::SPEED,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_04E < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    return super(attacker,opponent,hitnum,alltargets,showanimation) if @basedamage>0
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::SPATK,true,attacker)
    ...
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::SPATK,2,true,true,attacker)
    return ret ? 0 : -1
  end
  ...
  def pbAdditionalEffect(attacker,opponent)
    ...
    if opponent.pbCanReduceStatStage?(PBStats::SPATK,false,attacker)
      opponent.pbReduceStat(PBStats::SPATK,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_04F < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    return -1 if !opponent.pbCanReduceStatStage?(PBStats::SPDEF,true,attacker)
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    ret=opponent.pbReduceStat(PBStats::SPDEF,2,true,true,attacker)
    return ret ? 0 : -1
  end

  def pbAdditionalEffect(attacker,opponent)
    if opponent.pbCanReduceStatStage?(PBStats::SPDEF,false,attacker)
      opponent.pbReduceStat(PBStats::SPDEF,2,true,true,attacker)
    end
    return true
  end
end
...
class PokeBattle_Move_060 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    elsif @battle.field.effects[PBEffects::GrassyTerrain]>0 && !(attacker.isAirborne?)
      type=getConst(PBTypes,:GRASS)
    elsif  @battle.field.effects[PBEffects::MistyTerrain]>0 && !(attacker.isAirborne?)
...
class PokeBattle_Move_061 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.type1==getConst(PBTypes,:WATER) && opponent.type2==getConst(PBTypes,:WATER)
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
...
class PokeBattle_Move_074 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if opponent.pbPartner && !opponent.pbPartner.isFainted? && !opponent.pbParnter.hasWorkingAbility(:MAGICGUARD)
...
class PokeBattle_Move_079 < PokeBattle_Move
  def pbBaseDamageMultiplier(damagemult,attacker,opponent)
    if @battle.doublebattle
      ...
      if partnermove && attacker.pbPartner.hasMovedThisRound? && partnermove.function==0x7A
        return (damagemult*2.0).round
      end
    end
    return damagemult
...
class PokeBattle_Move_07A < PokeBattle_Move
  def pbBaseDamageMultiplier(damagemult,attacker,opponent)
    if @battle.doublebattle
      partnermove=@battle.choices[attacker.pbPartner.index][2]
      if partnermove && attacker.pbPartner.hasMovedThisRound? && partnermove.function==0x79
        return (damagemult*2.0).round
      end
    end
    return damagemult
...
class PokeBattle_Move_083 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    ...
    if attacker.pbPartner
      partnermove=@battle.choices[attacker.pbPartner.index][2]
      if partnermove.function==0x83
        if !attacker.pbPartner.hasMovedThisRound?
          attacker.pbPartner.effects[PBEffects::ForceMove]=true if !attacker.pbPartner.isFainted?
        else
          basedmg*=2
        end
      end
...
class PokeBattle_Move_084 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    if opponent.hasMovedThisRound? && opponent.lastMoveUsed<=0 &&
        PokeBattle_Move.pbFromPBMove(@battle,PBMove.new(opponent.lastMoveUsed)).basedamage>0
      basedmg*=2
    end
    return basedmg
  end
...
class PokeBattle_Move_085 < PokeBattle_Move
  def pbBaseDamageMultiplier(damagemult,attacker,opponent)
    if attacker.pbOwnSide.effects[PBEffects::LastRoundFainted]==@battle.turncount-1
      return (damagemult*2.0).round
    end
    return damagemult
  end
...
class PokeBattle_Move_086 < PokeBattle_Move
  def pbBaseDamage(basedmg,attacker,opponent)
    ...
    if attacker.item==0
      return basedmg*2
    elsif pbIsGem?(attacker.item)
      return basedmg*2 if (attacker.item==getID(PBItems,:NORMALGEM) && type==getConst(PBTypes,:NORMAL)) ||
        (attacker.item==getID(PBItems,:FIGHTINGGEM) && @type==getConst(PBTypes,:FIGHTING)) ||
        (attacker.item==getID(PBItems,:FLYINGGEM) && @type==getConst(PBTypes,:FLYING)) ||
        (attacker.item==getID(PBItems,:POISONGEM) && @type==getConst(PBTypes,:POISON)) ||
        (attacker.item==getID(PBItems,:GROUNDGEM) && @type==getConst(PBTypes,:GROUND)) ||
        (attacker.item==getID(PBItems,:ROCKGEM) && @type==getConst(PBTypes,:ROCK)) ||
        (attacker.item==getID(PBItems,:BUGGEM) && @type==getConst(PBTypes,:BUG)) ||
        (attacker.item==getID(PBItems,:GHOSTGEM) && @type==getConst(PBTypes,:GHOST)) ||
        (attacker.item==getID(PBItems,:STEELGEM) && @type==getConst(PBTypes,:STEEL)) ||
        (attacker.item==getID(PBItems,:FIREGEM) && @type==getConst(PBTypes,:FIRE)) ||
        (attacker.item==getID(PBItems,:WATERGEM) && @type==getConst(PBTypes,:WATER)) ||
        (attacker.item==getID(PBItems,:GRASSGEM) && @type==getConst(PBTypes,:GRASS)) ||
        (attacker.item==getID(PBItems,:ELECTRICGEM) && @type==getConst(PBTypes,:ELECTRIC)) ||
        (attacker.item==getID(PBItems,:PSYCHICGEM) && @type==getConst(PBTypes,:PSYCHIC)) ||
        (attacker.item==getID(PBItems,:ICEGEM) && @type==getConst(PBTypes,:ICE)) ||
        (attacker.item==getID(PBItems,:DRAGONGEM) && @type==getConst(PBTypes,:DRAGON)) ||
        (attacker.item==getID(PBItems,:DARKGEM) && @type==getConst(PBTypes,:DARK)) ||
        (attacker.item==getID(PBItems,:FAIRYGEM) && @type==getConst(PBTypes,:FAIRY))
    end
    return basedmg
  end
end
...
=begin  #Removed since Gen 6 Hidden Power is always 60
  def pbBaseDamage(basedmg,attacker,opponent)
    hp=pbHiddenPower(attacker.iv)
    return hp[1]
  end
...
class PokeBattle_Move_09D < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    for i in 0...4
      if attacker.battle.battlers[i].pbOwnSide.effects[PBEffects::MudSport]>0
        @battle.pbDisplay(_INTL("But it failed!"))
        return -1
      end
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    attacker.pbOwnSide.effects[PBEffects::MudSport]=5
...
class PokeBattle_Move_09E < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    for i in 0...4
      if attacker.battle.battlers[i].pbOwnSide.effects[PBEffects::WaterSport]>0
        @battle.pbDisplay(_INTL("But it failed!"))
        return -1
      end
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    attacker.pbOwnSide.effects[PBEffects::WaterSport]=5
...
class PokeBattle_Move_0A1 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !@battle.pbIsOpposing?(attacker.index)
      @battle.pbDisplay(_INTL("Lucky Chant shielded your team from critical hits!"))
    else
      @battle.pbDisplay(_INTL("Lucky Chant shielded the opposing team from critical hits!"))
    end  
    return 0
  end
end
...
class PokeBattle_Move_0A4 < PokeBattle_Move
  def pbAdditionalEffect(attacker,opponent)
    if @battle.field.effects[PBEffects::ElectricTerrain]>0 && !(attacker.isAirborne?)
        ...
        return false if !opponent.pbCanSleep?(true,false,self,attacker)
        opponent.pbSleep(attacker)
        @battle.pbDisplay(_INTL("{1} fell asleep!",opponent.pbThis))
        return true
    elsif @battle.field.effects[PBEffects::MistyTerrain]>0 && !(attacker.isAirborne?)
        return false if !opponent.pbCanReduceStatStage?(PBStats::SPATK,1,false,attacker)
        opponent.pbReduceStat(PBStats::SPATK,1,true,true,false,attacker)
        return true
    end
    case @battle.environment
      when PBEnvironment::Grass, PBEnvironment::TallGrass
        return false if !opponent.pbCanSleep?(false,false,false,self,attacker)
        opponent.pbSleep
        @battle.pbDisplay(_INTL("{1} went to sleep!",opponent.pbThis))  
      when PBEnvironment::MovingWater
        return false if !opponent.pbCanReduceStatStage?(PBStats::ATTACK,1,false,attacker)
        opponent.pbReduceStat(PBStats::ATTACK,1,true,true,false,attacker)
      when PBEnvironment::StillWater
        return false if !opponent.pbCanReduceStatStage?(PBStats::SPEED,1,false,attacker)
        opponent.pbReduceStat(PBStats::SPEED,1,true,true,false,attacker)
      when PBEnvironment::Underwater
        return false if !opponent.pbCanReduceStatStage?(PBStats::DEFENSE,1,false,attacker)
        opponent.pbReduceStat(PBStats::DEFENSE,1,true,true,false,attacker)
      ...
      when PBEnvironment::Sand
        return false if !opponent.pbCanReduceStatStage?(PBStats::ACCURACY,1,false,attacker)
        opponent.pbReduceStat(PBStats::ACCURACY,1,true,true,false,attacker)
      when pbRandom(100)<30  #Distinguish between wifi battles and plain terrain?
        return false if !opponent.pbCanParalyze?(false,self,attacker)
        opponent.pbParalyze(attacker)
        @battle.pbDisplay(_INTL("{1} is paralyzed!  It may be unable to move!",opponent.pbThis))
    end
...
class PokeBattle_Move_0AA < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    for poke in @battle.battlers
      count+=1 if poke.hasMovedThisRound?
    end
    if count==@battle.battlers.length-1
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
...
class PokeBattle_Move_0AB < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    @battle.pbDisplay(_INTL("Quick Guard protected your team!",attacker.pbThis))
    return 0
  end
end
...
class PokeBattle_Move_0AC < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if attacker.pbOwnSide.effects[PBEffects::WideGuard] || !@battle.doublebattle
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,nil,hitnum,alltargets,showanimation)
    attacker.pbOwnSide.effects[PBEffects::WideGuard]=true
    @battle.pbDisplay(_INTL("Wide Guard protected your team!",attacker.pbThis))
    return 0
  end
end
...
class PokeBattle_Move_0AD < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if isConst?(@id,PBMoves,:HYPERSPACEFURY)
      if attacker.pbCanReduceStatStage?(PBStats::DEFENSE)
        attacker.pbReduceStat(PBStats::DEFENSE,1,true)
      end
    end
    return ret
  end
...
class PokeBattle_Move_0AF < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @battle.lastMoveUsed>=0
      copymove=PBMoveData.new(@battle.lastMoveUsed)
      if (copymove.flags&0x10)!=0 && 
          ![getID(PBMoves,:COPYCAT),getID(PBMoves,:MIRRORMOVE)].include?(copymove.id)
        attacker.pbUseMoveSimple(@battle.lastMoveUsed,-1,opponent.index)
        return 0
      end
    end
    @battle.pbDisplay(_INTL("But it failed!"))
...
class PokeBattle_Move_0B0 < PokeBattle_Move
  ...
  def pbMoveFailed(attacker,opponent)
    if @battle.choices[attacker.index][2] && @battle.choices[attacker.index][2].id>0
        targets=[]
        attacker.pbFindUser(@battle.choices[attacker.index],targets)
        return true if targets.length==0 #fail if no targets
        opp=@battle.battlers[targets[0].index]
        oppmove=@battle.choices[opp.index][2]
        if oppmove && !opp.hasMovedThisRound? && oppmove.id>0 && 
          ![getID(PBMoves,:CHATTER),getID(PBMoves,:COUNTER),getID(PBMoves,:COVET),
          getID(PBMoves,:FOCUSPUNCH),getID(PBMoves,:MEFIRST),getID(PBMoves,:METALBURST),
          getID(PBMoves,:MIRRORCOAT),getID(PBMoves,:STRUGGLE),getID(PBMoves,:THIEF)].include?(oppmove.id)
            @battle.choices[attacker.index][2]=oppmove
            attacker.effects[PBEffects::MeFirst]=true
            attacker.pbUseMoveSimple(@battle.choices[attacker.index][2].id,-1,opponent.index)
            return false
        end
    end
    return true  
...
class PokeBattle_Move_0B3 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @battle.field.effects[PBEffects::ElectricTerrain]>0 && !(attacker.isAirborne?)
      move=getConst(PBMoves,:THUNDERBOLT)
    elsif @battle.field.effects[PBEffects::GrassyTerrain]>0 && !(attacker.isAirborne?)
      move=getConst(PBMoves,:ENERGYBALL)
    elsif @battle.field.effects[PBEffects::MistyTerrain]>0 && !(attacker.isAirborne?)
      move=getConst(PBMoves,:MOONBLAST)      
    end
    if move==0
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
...
class PokeBattle_Move_0B7 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if @battle.pbCheckSideAbility(:AROMAVEIL,opponent)
      @battle.pbDisplay(_INTL("{1} is protected by Aroma Veil!",opponent.pbThis))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_0CE < PokeBattle_Move
  def pbMoveFailed(attacker,opponent)
    ret=false
    ret=true if @battle.field.effects[PBEffects::Gravity]>0
    ret=true if opponent.effects[PBEffects::Substitute]>0
    ret=true if opponent.isAirborne?(true)
    ret=true if attacker.pbPartner==opponent #can't target ally
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if pbMoveFailed(attacker,opponent)
      @battle.pbDisplay(_INTL("But it failed!"))
      attacker.pbCancelMoves
      return -1
    end
    if attacker.effects[PBEffects::TwoTurnAttack]>0
      pbShowAnimation(@id,attacker,opponent,1,alltargets,showanimation) # Charging anim
      @battle.pbDisplay(_INTL("{1} flew {2} up into the air!",attacker.pbThis,opponent.pbThis(true)))
...
class PokeBattle_Move_0CF < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !opponent.isFainted? && opponent.damagestate.calcdamage>0 &&
       !opponent.damagestate.substitute && !(opponent.pbHasType?(:GHOST))
      if opponent.effects[PBEffects::MultiTurn]==0
        ...
        if isConst?(@id,PBMoves,:BIND)
          ...
          @battle.pbDisplay(_INTL("{1} became trapped by swirling magma!",opponent.pbThis))
          ...
          @battle.pbDisplay(_INTL("{1} has been afflicted with infestation by {2}!",opponent.pbThis,attacker.pbThis(true)))
        else
          @battle.pbDisplay(_INTL("{1} was trapped in the vortex!",opponent.pbThis))
        end
      end
...
class PokeBattle_Move_0D9 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.pbCanSleep?(true,true,true,self,attacker)
      return -1
    end
    if attacker.hp==attacker.totalhp
      @battle.pbDisplay(_INTL("{1}'s HP is full!",attacker.pbThis))
...
class PokeBattle_Move_0E0 < PokeBattle_Move
  def pbOnStartUse(attacker)
    ...
    if bearer && !attacker.hasWorkingAbility(:MOLDBREAKER)
      @battle.pbDisplayEffect(bearer)
      if EFFECTMESSAGES
        @battle.pbDisplay(_INTL("{1} cannot use {2}!",attacker.pbThis,@name))
      else
...
class PokeBattle_Move_0E1 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    attacker.effects[PBEffects::FinalGambit] = true
    return pbEffectFixedDamage(attacker.hp,attacker,opponent,hitnum,alltargets,showanimation)
  end
...
class PokeBattle_Move_0E2 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !prevented && opponent.pbOwnSide.effects[PBEffects::Mist]>0
      @battle.pbDisplay(_INTL("{1} is protected by the mist!",opponent.pbThis))
      prevented=true
    end
    ...
    if !prevented
      pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
      showanim=true
      if opponent.pbReduceStat(PBStats::ATTACK,2,true,showanim,false,attacker)
        ret=0; showanim=false
      end
      if opponent.pbReduceStat(PBStats::SPATK,2,true,showanim,false,attacker)
        ret=0; showanim=false
      end
    end
...
class PokeBattle_Move_0E5 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    for i in 0...4
      if @battle.battlers[i].effects[PBEffects::PerishSong]==0
        if @battle.battlers[i].hasWorkingAbility(:SOUNDPROOF) && !attacker.hasBypassingAbility
          @battle.pbDisplayEffect(opponent)
...
class PokeBattle_Move_0EB < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if opponent.hasWorkingAbility(:SUCTIONCUPS) && !attacker.hasBypassingAbility
      @battle.pbDisplayEffect(opponent)
      if EFFECTMESSAGES
        @battle.pbDisplay(_INTL("{1} anchored itself!",opponent.pbThis))
      else
...
class PokeBattle_Move_0EC < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.isFainted? && !opponent.isFainted? &&
       ...
       (!opponent.hasWorkingAbility(:SUCTIONCUPS) || !attacker.hasBypassingAbility) &&
      ...
      if !@battle.opponent
        ...
        opponent.effects[PBEffects::ForceSwitch]=true
      end
    end
    return ret
  end
...
class PokeBattle_Move_0F0 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !opponent.isFainted? && opponent.damagestate.calcdamage>0 &&
      ...
      if opponent.hasWorkingAbility(:STICKYHOLD) && !attacker.hasBypassingAbility
        @battle.pbDisplayEffect(opponent)
        abilityname=PBAbilities.getName(opponent.ability)
...
class PokeBattle_Move_0F5 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if !attacker.isFainted? && opponent.damagestate.calcdamage>0 &&
       !opponent.damagestate.substitute && (pbIsBerry?(opponent.item) || pbIsGem?(opponent.item))
      item=opponent.item
      itemname=PBItems.getName(item)
      opponent.item=0
      opponent.pokemon.itemInitial=0 if opponent.pokemon.itemInitial==item
      @battle.pbDisplay(_INTL("{1}'s {2} was burned up!",opponent.pbThis,itemname))
    end
    return ret
  end
end
...
class PokeBattle_Move_0F9 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if @battle.field.effects[PBEffects::MagicRoom]>0
      ...
      @battle.pbDisplay(_INTL("Magic Room wore off, and held items' effects returned to normal!"))
    else
      pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_0FF < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if @battle.weather==PBWeather::SUNNYDAY || @battle.isPrimordialWeather?
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_101 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if @battle.weather==PBWeather::SANDSTORM || @battle.isPrimordialWeather?
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_102 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    if @battle.weather==PBWeather::HAIL || @battle.isPrimordialWeather?
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
...
class PokeBattle_Move_103 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !@battle.pbIsOpposing?(attacker.index)
      @battle.pbDisplay(_INTL("Spikes were scattered all around the feet of the opposing team!"))
    else
...
class PokeBattle_Move_10D < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.pbHasType?(:GHOST)
      lowerspeed=attacker.pbCanReduceStatStage?(PBStats::SPEED,false,true,attacker)
      raiseatk=attacker.pbCanIncreaseStatStage?(PBStats::ATTACK,false,attacker)
      raisedef=attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false,attacker)
      if !lowerspeed && !raiseatk && !raisedef
        ...
        showanim=true
        if raiseatk
          attacker.pbIncreaseStat(PBStats::ATTACK,1,false,showanim,false,attacker)
          showanim=false
        end
        if raisedef
          attacker.pbIncreaseStat(PBStats::DEFENSE,1,false,showanim,false,attacker)
          showanim=false
        end
      end
    else
  ...
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !attacker.isFainted? && opponent.damagestate.calcdamage>0
      ...
      if attacker.effects[PBEffects::LeechSeed]>=0
        ...
        @battle.pbDisplay(_INTL("{1} was freed from Leech Seed!",attacker.pbThis))   
      end
      if attacker.pbOwnSide.effects[PBEffects::StealthRock]
        attacker.pbOwnSide.effects[PBEffects::StealthRock]=false
        if !@battle.pbIsOpposing?(attacker.index)
...
class PokeBattle_Move_112 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    showanim=true
    if attacker.pbCanIncreaseStatStage?(PBStats::DEFENSE,false)
      attacker.pbIncreaseStat(PBStats::DEFENSE,1,true,showanim)
      attacker.effects[PBEffects::StockpileDef]+=1
      showanim=false
    end
    if attacker.pbCanIncreaseStatStage?(PBStats::SPDEF,false)
      attacker.pbIncreaseStat(PBStats::SPDEF,1,true,showanim)
    ...
    if attacker.effects[PBEffects::StockpileDef]>0
      if attacker.pbCanReduceStatStage?(PBStats::DEFENSE,false,true,attacker)
        attacker.pbReduceStat(PBStats::DEFENSE,attacker.effects[PBEffects::StockpileDef],
           true,showanim,true,attacker)
        showanim=false
      end
    end
    if attacker.effects[PBEffects::StockpileSpDef]>0
      if attacker.pbCanReduceStatStage?(PBStats::SPDEF,true,true,attacker)
...
class PokeBattle_Move_116 < PokeBattle_Move
  def pbMoveFailed(attacker,opponent)
    return true if @battle.choices[opponent.index][0]!=1
    opponentmove=@battle.choices[opponent.index][2]
...
class PokeBattle_Move_11D < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if !opponent.hasMovedThisRound?
      opponent.effects[PBEffects::ForceMove]=true
      @battle.pbDisplay(_INTL("{1} helped {2} move next!",attacker.pbThis,opponent.pbThis))
    else
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
...
class PokeBattle_Move_11E < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ret=super(attacker,opponent,hitnum,alltargets,showanimation)
    if !opponent.effects[PBEffects::Quash] && !opponent.hasMovedThisRound?
      opponent.effects[PBEffects::Quash]=true
      @battle.pbDisplay(_INTL("{1} was quashed!",opponent.pbThis))
    else
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
...
class PokeBattle_Move_11F < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
      @battle.field.effects[PBEffects::TrickRoom] = (@battle.field.effects[PBEffects::TrickRoom]==0) ? 5 : 0
...
=begin     don't need this?
  def pbMoveFailed(attacker,opponent)
    if !@battle.doublebattle || !(attacker.pbPartner)
      return false
    else
      return true
    end
...
=end
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    tempbattler=attacker
    attacker=opponent
    opponent=tempbattler
    pbShowAnimation(@id,attacker,opponent,hitnum,alltargets,showanimation)
    attacker.pbUpdate(true)
    opponent.pbUpdate(true)
    @battle.pbDisplay(_INTL("{1} switched places with its ally!",opponent.pbThis))
  end
end
...
class PokeBattle_Move_124 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
      @battle.field.effects[PBEffects::WonderRoom] = (@battle.field.effects[PBEffects::WonderRoom]==0) ? 5 : 0
...
################################################################################
# User increases its attack stat by 2 stages if opponent faints.
################################################################################
...
class PokeBattle_Move_144 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    @battle.pbDisplay(_INTL("{2} type was added to {1}!",opponent.pbThis,typename))
    return 0
  end
end
...
class PokeBattle_Move_154 < PokeBattle_Move
  def pbEffect(attacker,opponent,hitnum=0,alltargets=nil,showanimation=true)
    ...
    if count>0
      @battle.pbDisplay(_INTL("{1}'s stat changes were all reversed!",opponent.pbThis))
    else
      @battle.pbDisplay(_INTL("But it failed!"))
      return -1
    end
