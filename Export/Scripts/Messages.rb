...
def Kernel.pbShowCommands(msgwindow,commands=nil,cmdIfCancel=0,defaultCmd=0)
  ...
  if commands
    ...
    loop do
      ...
      if $MessageSprites
        for i in $MessageSprites.keys
          $MessageSprites[i].update
        end
      end
      pbUpdateSceneMap
    end
...
$MessageSprites=nil
def Kernel.setMessageSprites(sprites)
...
def Kernel.pbMessageDisplay(msgwindow,message,letterbyletter=true,commandProc=nil,sprites=nil)
  sprites=$MessageSprites if !($MessageSprites.nil?)
  return if !msgwindow
